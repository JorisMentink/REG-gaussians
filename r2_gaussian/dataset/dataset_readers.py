...
def readCTameras(meta_data, source_path, eval=False, scene_scale=1.0):
    ...
    for split in splits:
        ...
        for i_split in range(n_split):
            ...
            phasebin = frame_info.get("phasebin", None)
            timepoint = frame_info.get("timepoint", None)
            gt_vol_path = frame_info.get("gt_vol", None)
            if gt_vol_path is not None:
                gt_vol_path = osp.join(source_path, gt_vol_path)
...
class DynSceneInfo(NamedTuple):
    train_cameras: list
    test_cameras: list
    gt_vols: list[torch.tensor]
    scanner_cfg: dict
    scene_scale: float

def read_dyn_BlenderInfo(path, eval):
    """Read blender format CT data."""
    # Read meta data
    meta_data_path = osp.join(path, "meta_data.json")
    with open(meta_data_path, "r") as handle:
        meta_data = json.load(handle)
    meta_data["gt_vols"] = [osp.join(path, v) for v in meta_data["gt_vols"]]
    ...
    gt_vols = [torch.from_numpy(np.load(v)).float().cuda() for v in meta_data["gt_vols"]]

    scene_info = DynSceneInfo(
        train_cameras=train_cam_infos,
        test_cameras=test_cam_infos,
        gt_vols=gt_vols,
        scanner_cfg=meta_data["scanner"],
...
sceneLoadTypeCallbacks = {
    ...
    "NAF": readNAFInfo,
    "DynBlender": read_dyn_BlenderInfo,
}